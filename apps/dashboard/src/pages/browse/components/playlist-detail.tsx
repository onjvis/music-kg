...
export const PlaylistDetail = () => {
  ...
  useEffect(() => {
    if (playlist.id && origin) {
      getSparqlLinks(playlist.id, origin as DataOrigin).then((links) => setLinks(links));
    }
  }, [playlist.id, origin]);
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {links?.length > 0 && <EntitySameAsLinks entityRoutePrefix={AppRoute.BROWSE_PLAYLISTS} links={links} />}
    </div>
  );
};
