...
export const deleteRecording = async (id: string, origin: DataOrigin): Promise<void> => {
  const originPrefix: string = getPrefixFromOrigin(origin);
  const recordingSubject: IriTerm = iriWithPrefix(originPrefix, id);

  const query: string = createDeleteQuery({ graph: prefix2graph(originPrefix), subject: recordingSubject });
