...
export const getAlbumFromWikidataDump = async (externalUrl: string): Promise<SparqlEntity> => {
  const externalUrlParts: string[] = externalUrl.split('/');
  const entityId: string = externalUrlParts[externalUrlParts.length - 1];
  const query: string = createGetFromWikidataQuery({ entityType: 'album', entityId });
